...
def checkArguments(stringParameters, parameters, kwargs):
...
class Query(ObjectType):
    ...
    def resolve_getCharacter(_, info, **kwargs):
        ...
        checkArguments(stringParameters, parameters, kwargs)

        character = getData.getCharacter(name=parameters[0], nameStartsWith=parameters[1],
                                         modifiedSince=parameters[2], comics=parameters[
                                             3], series=parameters[4], events=parameters[5],
    ...
    def resolve_getComic(_, info, **kwargs):
        ...
        stringParameters = ['title', 'titleStartsWith', 'startYear', 'issueNumber',
                            'diamondCode', 'upc', 'isbn', 'ean', 'issn', 'hasDigitalIssue', 'modifiedSince', 'creators', 'characters', 'series', 'events', 'stories', 'sharedAppearances',
    ...
    def resolve_getCreator(_, info, **kwargs):
        ...
        creator = getData.getCreator(firstName=parameters[0], middleName=parameters[1], lastName=parameters[2],
                                     suffix=parameters[3], nameStartsWith=parameters[
                                         4], firstNameStartsWith=parameters[5], middleNameStartsWith=parameters[6],
                                     lastNameStartsWith=parameters[7], modifiedSince=parameters[
                                         8], comics=parameters[9], series=parameters[10],
    ...
    def resolve_getEvent(_, info, **kwargs):
        ...
        stringParameters = ['name', 'nameStartsWith', 'modifiedSince', 'creators',
        ...
        event = getData.getEvent(name=parameters[0], nameStartsWith=parameters[1], modifiedSince=parameters[2],
                                 creators=parameters[3], characters=parameters[4], series=parameters[
                                     5], comics=parameters[6], stories=parameters[7],
    ...
    def resolve_getSeries(_, info, **kwargs):
        ...
        parameters = [title, titleStartsWith, startYear, modifiedSince, comics, stories,
                      events, creators, characters, seriesType, contains,
    ...
    def resolve_getStory(_, info, **kwargs):
        ...
        parameters = [modifiedSince, comics, series, events,
                      creators, characters, orderBy, limit, offset]
